...
const fetchEventsData = async () => { 
  try {
    ...
    if (!res.ok) {
      ...
      return { error: true, message: `イベントデータの取得に失敗しました (${res.statusText})` };
    }
...
const fetchWorksData = async () => { 
  try {
    ...
    if (!res.ok) {
      ...
      return { error: true, message: `作品データの取得に失敗しました (${res.statusText})` };
    }
    return await res.json();
  } catch (error) {
  ...
  return (  
    <div>  
      <Head>  
        ...
        <meta  
          name="description"  
          content={event?.eventname ? `${event.eventname}の作品一覧です。` : "イベント一覧です。"}  
        />  
      </Head>  
      <Header />  
      <div className="content">  
...
export const getStaticProps = async ({ params }) => { 
  ...
  const event = eventsData.find((event) => event.eventid === id) || {}; // イベントが見つからない場合は空のオブジェクトを返す 
