...
const getLatestWorkByTlink = (icons, tlink) => { 
  const filteredIcons = icons.filter(icon => icon.tlink?.toLowerCase() === tlink.toLowerCase()); 
  return filteredIcons.length > 0 ? filteredIcons[0] : null; // 最新の作品を取得 
}; 
...
export default function UserPage({ users = [], icons = [] }) {           
  ...
  const sortedUsers = [...users].sort((a, b) => {  
    ...
    if (sortOption === 'totalWorks') {  
      const aTotalWorks = icons.filter(icon =>   
        (icon.tlink && icon.tlink.toLowerCase() === a.username.toLowerCase()) ||  
        (icon.memberid && icon.memberid.toLowerCase().includes(a.username.toLowerCase()))  
      ).length;  
      const bTotalWorks = icons.filter(icon =>   
        (icon.tlink && icon.tlink.toLowerCase() === b.username.toLowerCase()) ||  
        (icon.memberid && icon.memberid.toLowerCase().includes(b.username.toLowerCase()))  
      ).length;  
        ...
        <div className={styles.userlist}>           
          {sortedUsers.length > 0 ? (           
            sortedUsers.map((user) => {           
              ...
              const matchingWorksCount1 = icons.filter(icon =>   
                icon.tlink && icon.tlink.toLowerCase() === user.username.toLowerCase()  
              ).length;   
              const matchingWorksCount2 = icons.filter(icon =>    
                (icon.memberid && icon.memberid.toLowerCase().includes(user.username.toLowerCase()))   
              ).length;   
              const matchingWorksCount3 = matchingWorksCount1 + matchingWorksCount2;  
 
              return (           
                <div className={styles.users} key={user.username} >           
                  {latestWork && latestWork.icon ? (           
                    <Image           
                      src={`https://lh3.googleusercontent.com/d/${latestWork.icon.slice(33)}`}          
                      className="icon"           
                  ...
                  <Link href={`/user/${user.username}`} passHref>    
                    ...
                    <p>個人作品数: {matchingWorksCount1}</p>   
                    <p>合作作品数: {matchingWorksCount2}</p>   
                    <p>合計作品数: {matchingWorksCount3}</p>   
                  </Link>      
                </div>           
              );           
            })           
