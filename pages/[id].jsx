...
const getVideoThumbnail = async (videoId) => {
  ...
  try {
    ...
    if (videoData.items.length > 0) {
      ...
      return thumbnails?.medium?.url || `https://i.ytimg.com/vi/${videoId}/sddefault.jpg`; // mediumを使用
    }
...
export default function WorkId({ work, previousWorks, nextWorks, thumbnail, previousThumbnails, nextThumbnails }) { 
...
export async function getStaticProps({ params }) {
  ...
  const previousThumbnails = await Promise.all(
    previousWorks.map((prevWork) => getVideoThumbnail(prevWork.ylink.slice(17, 28)))
  );
  ...
  return {
    props: { work, previousWorks, nextWorks, thumbnail, previousThumbnails, nextThumbnails },
  };
}
